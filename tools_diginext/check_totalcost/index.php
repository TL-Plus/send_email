...
<head>
    ...
    <title>DIGINEXT | TOTAL COST</title>
...
<body class="container mt-3">
    <div class="row mt-3 mb-5">
        ...
        <div class="col-md-10 text-center mt-3">
            <h1 class="title-table mb-4 mx-auto">Check Total Cost</h1>
            <h4 class="text-center">Kiểm tra tổng cước trong tháng của CDR và DCN</h4>
        </div>
    </div>
    ...
    <div class="row mt-5">
        ...
        <div class="col-md-6">
            ...
            <!-- check-data-dcn -->
            <form method="POST" class="mt-4" id="check-data-dcn">
                <input type="hidden" name="check_type" value="dcn"> <!-- Thêm tham số ẩn -->

                <div class="form-group">
                    <label for="start_at_dcn">Start At:</label>
                    <?php
                    // Tạo giá trị mặc định là ngày đầu của tháng
                    $start_at_default = isset($_POST['start_at_dcn']) ? $_POST['start_at_dcn'] : date('Y-m-01');
                    ?>
                    <input type="date" name="start_at_dcn" id="start_at_dcn" class="form-control" required
                        placeholder="Enter Start At" step="1" value="<?php echo $start_at_default; ?>">
                </div>
                <div class="form-group">
                    <label for="end_at_dcn">End At:</label>
                    <?php
                    // Tạo giá trị mặc định là ngày hiện tại
                    $end_at_default = isset($_POST['end_at_dcn']) ? $_POST['end_at_dcn'] : date('Y-m-d');
                    ?>
                    <input type="date" name="end_at_dcn" id="end_at_dcn" class="form-control" required
                        placeholder="Enter End At" step="1" value="<?php echo $end_at_default; ?>">
                </div>

                <button type="submit" name="check_data_dcn" class="btn btn-primary mb-4">Check DCN</button>
            </form>

            <?php
            // Chỉ chạy include khi biến POST "check_type" có giá trị là "dcn"
            if (isset($_POST['check_type']) && $_POST['check_type'] === 'dcn') {
                include 'includes/body_data_dcn.php';
            }
            ?>
